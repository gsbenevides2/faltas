...
export async function FaltaMenuRemover() {
  ...
  const faltas_choices = faltas.map((falta) => {
    const text = falta.getData().toLocaleDateString();
    return {
      name: text,
      value: text,
      falta,
  ...
  const falta_answers = await inquirer.prompt<{ falta: string }>([
    {
      type: "list",
      name: "falta",
      message: "Qual falta deseja remover?",
      choices: faltas_choices,
    },
  ]);

  const falta = faltas_choices.find(
    (choice) => choice.value === falta_answers.falta
  )?.falta;

  if (!falta) return;

  await falta.remover_falta();

  console.log(chalk.green("Falta removida com sucesso!"));
}
